...
<table id="indexTable" class="table table-striped table-hover ">
  <thead>
    <tr>
        ...
        <th class="no-sort">TotalUnsettleBill</th>
        <th>StartPeriod</th>
        <th>EndPeriod</th>
        <th class="no-sort">Module</th>
        <th class="no-sort">PaymentMethod</th>
        <th class="no-sort">Amount</th>
        <th class="no-sort">PAMCubic</th>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    table = $('#indexTable').DataTable({
        ...
        "ajax": {
          "url": "<?php echo site_url('pam/list_reactivate_pam') ?>",
          "type": "POST",
        },
        "columnDefs": [
...
function edit_record(id, custid) {
    save_method = 'update';
    $('#form')[0].reset(); // reset form on modals
    $('.form-group').removeClass('has-error'); // clear error class
    $('.help-block').empty(); // clear error string
    get_total_billing(custid);
}
...
function get_total_billing(custid) {
    $.ajax({
