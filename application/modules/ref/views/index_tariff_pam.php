...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		table = $('#indexTable').DataTable({
		    ...
		    "order": [],
		    "ajax": {
		      "url": "<?php echo site_url('ref/list_tariff_pam') ?>",
		      "type": "POST"
		    },
		    "columnDefs": [
	...
	function edit_record(id) {
		...
		$.ajax({
			url : "<?php echo site_url('ref/get_tariff_pam_by_id/')?>",
			type: "POST",
			data: {"Id": id},
			dataType: "JSON",
			success: function(data) {
				...
				$('#AmountPerCubic').val(toCurrencyFormat(data.AmountPerCubic));
				$('#SubsAmount').val(toCurrencyFormat(data.Abonemen));
	...
	function delete_record(id, code) {
	    if(confirm('Are you sure delete tariff code : '+ code +'?')) {
	        $.ajax({
	            url : "<?php echo site_url('ref/delete_tariff_pam')?>",
	            type: "POST",
	            data: {"Id": id},
	            dataType: "JSON",
	            success: function(data) {
	...
	function save() {
	    ...
	    if(save_method == 'add') {
	        url = "<?php echo site_url('ref/add_tariff_pam')?>";
	    } else {
	        url = "<?php echo site_url('ref/edit_tariff_pam')?>";
	    }
                ...
                <form action="#" id="form" class="form-horizontal">
                    ...
                    <div class="form-body">
                        <div class="form-group">
                            ...
                            <label class="control-label col-md-3">Amount / m&sup3;</label>
                            <div class="col-md-9">
                                <input id="AmountPerCubic" name="AmountPerCubic" placeholder="0" class="form-control currency" type="text" maxlength="6">
                                <span class="help-block"></span>
                            </div>

                            <label class="control-label col-md-3">Abonemen</label>
                            <div class="col-md-9">
                                <input id="SubsAmount" name="SubsAmount" placeholder="0" class="form-control currency" type="text" maxlength="11">
                                <span class="help-block"></span>
                            </div>
